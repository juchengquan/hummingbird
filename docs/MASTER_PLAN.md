...
| Plan | Status | Sketch |
|---|---|---|
| 🪜 [Cross-product inspirations menu](PLAN-cross-product-inspirations.md) | 6 of 14 shipped, 1 closed | A menu, not a single feature — 14 items synthesised from a five-cohort survey of OSS + commercial AI/chat/agent products. **Shipped:** #1 conversation system prompt (#165), #2 Ollama + #3 OpenRouter + #5 Library (#167), #4 Part 2 gateway tagging (#166), #6 `#`-mention + RAG toggle (#168, #169). **Closed:** #4 Part 1 (caching — won't do without a Vercel-gateway commitment). **Open:** #7 Flowchat canvas (now has its own plan — see row below), #8 Beam, #9 Elicit tables, #10 Langfuse, #11 Aider editor pair, #12 hybrid search, #13 Letta memory, #14 LangGraph checkpointer. Per-item status + a Shipped tracker live in the plan |
| 📐 [Flowchat canvas](PLAN-flowchat-canvas.md) | planning | Wire `forkConversation` to land its result as a `conversation`-kind node on the workspace canvas, with **derived** fork edges (read at render time from `parentId` + `forkedFromMessageId`) in a distinct visual style. Canvas stays freely editable for positions and user-decoration edges; fork edges can't be hand-drawn (avoids the semantic muddle of fake fork-relationships). Item #7 from the inspirations menu, scope M, one PR planned across three commits |
