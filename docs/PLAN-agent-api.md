...
Status: **🪜 Phased — Phases 0 + 1 + 2a + 2b-1 + 2b-2 + 3a + 3b + 3c-2
+ 3d-1 shipped, Phases 3c-1 + 3d-2+ pending or in-flight (parallel
...
Phase 3a (`continue` action + chunk-break yield), Phase 3b (suspend
path + `respond` action), Phase 3c-2 (`searchFiles` tool with RLS
...
resumes the loop with seeded emitter seq/step. `searchFiles` is
config-gated on the new `ToolContext` (pool + user_id); calls the
`search_file_sections` Postgres RPC under `SET LOCAL ROLE
authenticated` + `request.jwt.claims = {sub: <user_id>, role:
'authenticated'}` so RLS on `files` evaluates against the user —
service-role pool can't bypass per-user visibility. `generateImage`
is config-gated on `MINIMAX_CN_API_KEY` (same env var the TS side
reads, so a single `.env` covers both); a narrow textual SSRF
blocklist guards `referenceImageUrl` until the full DNS-resolution
port lands. Phase 3d-2 (Supabase Storage persistence + signed URLs)
ships next on top. Phase 3c-1 (`webSearch` via Tavily) ships in a
parallel PR. Host decided (self-host on a small VM — see §Host
