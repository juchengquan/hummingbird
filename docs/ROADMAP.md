...
| 🪜 [Chat send-pipeline extraction](PLAN-chat-send-extraction.md) | Phases 1–2 shipped (#112) | Cleanup Phase 5 follow-up. Phase 1 pure attachment + message builders; Phase 2 the `useChatSend` hook (abort map, streaming flags, live tool-call buffer, mock fallback, 620-LOC pipeline). `chat.tsx` 1872 → 1179 (−693). Phase 3 (slim to <1100) + Phase 4 (smart-paste/dropzone) are scoped follow-ups |
