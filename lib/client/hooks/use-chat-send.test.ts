...
describe("translateFrame — legacy custom format passthrough", () => {
  test("text", () => {
    expect(translateFrame('{"type":"text","value":"hi"}')).toEqual({
      type: "text",
      value: "hi",
    })
  })
  ...
  test("reasoning", () => {
    expect(translateFrame('{"type":"reasoning","value":"hmm"}')).toEqual({
      type: "reasoning",
      value: "hmm",
    })
  })
  ...
  test("tool_call", () => {
    const out = translateFrame(
      '{"type":"tool_call","id":"c1","name":"webSearch","args":{"query":"x"}}',
    )
    expect(out).toEqual({
      type: "tool_call",
      id: "c1",
  ...
  test("tool_result with summary + results", () => {
    const out = translateFrame(
      '{"type":"tool_result","id":"c1","name":"webSearch","summary":"3 results","results":[{"title":"a","url":"https://a","snippet":""}]}',
    )
  ...
  test("tool_image", () => {
    const out = translateFrame(
      '{"type":"tool_image","id":"i1","mode":"t2i","images":[{"id":"i1-0","url":"https://x"}]}',
    )
  ...
  test("suggestions", () => {
    expect(translateFrame('{"type":"suggestions","values":["a","b"]}')).toEqual({
      type: "suggestions",
      values: ["a", "b"],
    })
  })
  ...
  test("error", () => {
    expect(translateFrame('{"type":"error","code":"provider","message":"boom"}')).toEqual({
      type: "error",
      code: "provider",
      message: "boom",
    })
  ...
  test("done", () => {
    expect(translateFrame('{"type":"done"}')).toEqual({ type: "done" })
  })
})
